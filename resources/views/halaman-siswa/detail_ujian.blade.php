    ...
    <div class="card my-5" style="border: 1px solid black;">
        <div class="card-body">
            ...
            <div class="row">
				...
				@if($soal->jenis == 'tes')
				<div class="col-md-10">: Tes Penempatan</div>
				@else
				<div class="col-md-10">: Ujian</div>
				@endif
            </div>
            <div class="row">
...
<script>
	  ...
	  if (remainingSeconds < 10) {
	   ...
	   if (seconds == 300) {
	   	...
	   	seconds--;
			...
			$.ajax({
				type: "POST",
				url: "{{ url('e-learning/ujian/kirim-jawaban') }}",
				data: {id_soal: id_soal},
				success: function(data) {
					window.location.href = "{{ url('e-learning/ujian/finish/'.$soal->id) }}";
				}
			})
	   } else {
	     seconds--;
	...
	$(document).ready(function(){
		...
		$('.no_soal').click(function() {
			...
			$.ajax({
				type: "GET",
				url: "{{ url('e-learning/ujian/get-soal') }}/"+id_soal,
				success: function(data) {
					$('#wrap-soal').html(data);
				}
			})
  	...
  	// 		<button type="button" class="btn" id="kirim-jawaban" data-id="{{ $soal->id }}" style="background-image: linear-gradient(to right, #1523f3 , #0495c1); border: none; color: #fff;"><i class="fa fa-check-circle" aria-hidden="true"></i> Iya! Saya Kirim Jawaban Saya Sekarang.</button>
		...
		$(document).on('click', ".jawab", function(){
			...
			$.ajax({
				type: "POST",
				url: "{{ url('e-learning/ujian/jawab') }}",
